    ...
    void run(const MatchFinder::MatchResult &Result) override {
        ...
        if (LHSVar->hasGlobalStorage() || LHSVar->isStaticLocal()) {
            ...
            llvm::errs() << Loc.getFilename() << ":" << Loc.getLine()
                         << ": [HSCAG.8.3] Address of local variable '"
                         << LocalVar->getName()
                         << "' stored in pointer '" << LHSVar->getName()
                         << "' which outlives it. When the local goes out of scope,"
                         << " the pointer becomes dangling and reading it is"
                         << " undefined behaviour.\n";
        }
    }
};
